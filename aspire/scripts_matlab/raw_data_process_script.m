...
%% PATH TO ASPIRE
addpath('/home/fpoitevi/Toolkit/aspire/')
initpath

%% PATH INFO TO DATA
star_path = '/scratch/users/fpoitevi/data/ftp.ebi.ac.uk/pub/databases/empiar/archive/10107/data/shiny_fp_short.star';
prefix = '/scratch/users/fpoitevi/data/ftp.ebi.ac.uk/pub/databases/empiar/archive/10107/data';

% PARAMETERS
num_pool = 32 ;       % number of workers
n = 1e2;              % number of images
n_noise = 1e2;        % number of images for noise estimation
downsampleddim = 330; % image size
c = 0.25;             % bandlimit (?)
R = 150;              % (?)
L0 = downsampleddim;  % (?) 
n_r = ceil(4*c*R);    % (?)
...
[ noise_v_r ] = estimate_noise_real(projs(:,:,1:n_noise)); % Variance of noise in prewhitened projections
% write 'mean' to mrc
mean_img = mean(projs, 3); % mean prewhitened image
WriteMRC(mean_img,pixA,'prewhitened_mean.mrc');
projs = cfft2(projs); % Images in fourier space
n_im = size(projs,3); % number of images
...
WriteMRC(projs,pixA,'projs.mrcs');
